...
global out_paths subconds num lim E_T_flag task_flag plot_hb %#ok<NUSED>
...
if rate_flag < 0 || rate_flag > length(rates) 
    error('rate_flag must be a 0:length(rates) integer, indicating calculate 5 different event rates, or only a specific on (1:length(rates)')
end

if mod(rate_flag,1) ~= 0
    error('rate_flag must a round integer between 0 and length(rates)')
end
