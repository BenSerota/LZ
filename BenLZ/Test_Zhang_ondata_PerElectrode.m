...
global conds subconds data_paths out_paths WS_path e N % s
DOC_basic

%% setting parameters:
Zthresh = 3;
% s = 4;  % # of subjects to check per cond
e = 400; % max # of epochs to check
N = numel(conds);
%% go
C_all_conds = allcondsLZC(conds,subconds,Zthresh);
cd('E:\Dropbox\Ben Serota\momentary\WS')
SaveUniqueName('LZC_ondata_PerElectrode')
...
C_task = nan(1,e); %n !
for i = 1:e %n
    C_task(i) = LZ_C_Zhang_PerElectrode(data_binary(:,:,i));
end
end
